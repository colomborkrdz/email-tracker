...
// Add notified column to emails table (idempotent)
try { db.prepare(`ALTER TABLE emails ADD COLUMN notified INTEGER NOT NULL DEFAULT 0`).run(); } catch {}
...
module.exports = {
  ...
  markEmailNotified: db.prepare('UPDATE emails SET notified = 1 WHERE track_id = ?'),
