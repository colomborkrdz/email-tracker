...
// Seed user — created on startup if not exists
const bcrypt = require('bcryptjs');
const crypto = require('crypto');
if (process.env.SEED_USER_EMAIL && process.env.SEED_USER_PASSWORD) {
  const existing = db.prepare('SELECT id FROM users WHERE email = ?').get(process.env.SEED_USER_EMAIL.toLowerCase().trim());
  if (!existing) {
    const hash = bcrypt.hashSync(process.env.SEED_USER_PASSWORD, 10);
    db.prepare(`
      INSERT INTO users (id, email, password_hash, email_verified, verification_token, created_at)
      VALUES (?, ?, ?, 1, NULL, ?)
    `).run(crypto.randomUUID(), process.env.SEED_USER_EMAIL.toLowerCase().trim(), hash, new Date().toISOString());
    console.log('[db] Seed user created:', process.env.SEED_USER_EMAIL);
  }
}
